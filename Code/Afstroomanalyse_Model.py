...
arcpy.env.workspace = r"C:\Users\tom_s\Desktop\afstroomanalyse\Utrecht_afstroomanalyse\Afstroomanalyse"
work_location = r"C:\Users\tom_s\Desktop\afstroomanalyse\Utrecht_afstroomanalyse\Afstroomanalyse"
...
Buurten = ['Wittevrouwen']

# Bepaal het volume van de regenbui (in meter)
Regenbui = 0.0374  # Bui volume in m
# Duur van de bui
uur = 1  # Tijd in uren
...
for x in Buurten:
    ...
    arcpy.CreateFolder_management(os.path.join(work_location, r"Buurten\{}").format(x),
                                  "resultaten")  # Maak nieuwe map 'resultaten'
    arcpy.CreateFolder_management(os.path.join(work_location, r"Buurten\{}").format(x),
                                  "tijdelijk")  # Maak nieuwe map 'tijdelijk'
    ...
    arcpy.Buffer_analysis(bk_30m_shp, os.path.join(work_location, r"Buurten\{}\tijdelijk\bk_30m_shp_buf.shp").format(x),
                          "100 meter", "FULL", "ROUND", "NONE")
    buf_temp = os.path.join(work_location, r"Buurten\{}\tijdelijk\bk_30m_shp_buf.shp").format(x)
    arcpy.Clip_analysis(os.path.join(work_location, r"Input_Afstroomanalyse\pandPolygon.shp"), buf_temp,
                        os.path.join(work_location, r"Buurten\{}\tijdelijk\pandPolygon_clip.shp").format(x))
    ...
    if pandPolygon_shp == '#' or not pandPolygon_shp:
        pandPolygon_shp = os.path.join(work_location, r"Buurten\{}\tijdelijk\pandPolygon_clip.shp").format(
            x)  # provide a default value if unspecified

    Water_30m_shp = arcpy.GetParameterAsText(4)
    if Water_30m_shp == '#' or not Water_30m_shp:
        Water_30m_shp = os.path.join(work_location, r"Buurten\{}\Water_30m_shp.shp").format(
            x)  # provide a default value if unspecified

    bk_zWater_30m_shp = arcpy.GetParameterAsText(7)
    if bk_zWater_30m_shp == '#' or not bk_zWater_30m_shp:
        bk_zWater_30m_shp = os.path.join(work_location, r"Buurten\{}\bk_zWater_30m_shp.shp").format(
            x)  # provide a default value if unspecified

    # Rest of Script arguments
    BK_AHN_mWmP = arcpy.GetParameterAsText(5)
    if BK_AHN_mWmP == '#' or not BK_AHN_mWmP:
        BK_AHN_mWmP = os.path.join(work_location, r"Buurten\{}\resultaten\BK_AHN_mWmP.tif").format(
            x)  # provide a default value if unspecified

    BK_AHN_mW = arcpy.GetParameterAsText(17)
    if BK_AHN_mW == '#' or not BK_AHN_mW:
        BK_AHN_mW = os.path.join(work_location, r"Buurten\{}\tijdelijk\BK_AHN_mW.tif").format(
            x)  # provide a default value if unspecified

    BK_streamOrder = arcpy.GetParameterAsText(9)
    if BK_streamOrder == '#' or not BK_streamOrder:
        BK_streamOrder = os.path.join(work_location, r"Buurten\{}\resultaten\BK_streamOrder.tif").format(
            x)  # provide a default value if unspecified

    BK_streamOrder_minWOS = arcpy.GetParameterAsText(9)
    if BK_streamOrder_minWOS == '#' or not BK_streamOrder_minWOS:
        BK_streamOrder_minWOS = os.path.join(work_location, r"Buurten\{}\resultaten\BK_streamOrder_minWOS.tif").format(
            x)  # provide a default value if unspecified

    mpzw_strorder = arcpy.GetParameterAsText(8)
    if mpzw_strorder == '#' or not mpzw_strorder:
        mpzw_strorder = os.path.join(work_location, r"Buurten\{}\resultaten\mpzw_strorder.tif").format(
            x)  # provide a default value if unspecified

    mpzw_strorder_minWOS = arcpy.GetParameterAsText(8)
    if mpzw_strorder_minWOS == '#' or not mpzw_strorder_minWOS:
        mpzw_strorder_minWOS = os.path.join(work_location, r"Buurten\{}\resultaten\pzw_strorder_minWOS.tif").format(
            x)  # provide a default value if unspecified

    mPzW_fd = arcpy.GetParameterAsText(11)
    if mPzW_fd == '#' or not mPzW_fd:
        mPzW_fd = os.path.join(work_location, r"Buurten\{}\tijdelijk\mpzw_fd.tif").format(
            x)  # provide a default value if unspecified

    mPzW_fd_minWOS = arcpy.GetParameterAsText(11)
    if mPzW_fd_minWOS == '#' or not mPzW_fd_minWOS:
        mPzW_fd_minWOS = os.path.join(work_location, r"Buurten\{}\tijdelijk\mpzw_fd_minWOS.tif").format(
            x)  # provide a default value if unspecified

    outIsNull = arcpy.GetParameterAsText(12)
    if outIsNull == '#' or not outIsNull:
        outIsNull = os.path.join(work_location, r"Buurten\{}\tijdelijk\outIsNull.tif").format(
            x)  # provide a default value if unspecified

    outRgnGrp = arcpy.GetParameterAsText(12)
    if outRgnGrp == '#' or not outRgnGrp:
        outRgnGrp = os.path.join(work_location, r"Buurten\{}\tijdelijk\outRgnGrp.tif").format(
            x)  # provide a default value if unspecified

    outRgnGrp_Basin = arcpy.GetParameterAsText(12)
    if outRgnGrp_Basin == '#' or not outRgnGrp_Basin:
        outRgnGrp_Basin = os.path.join(work_location, r"Buurten\{}\tijdelijk\outRgnGrp_Basin.tif").format(
            x)  # provide a default value if unspecified
    ...
    if panden == '#' or not panden:
        panden = os.path.join(work_location, r"Buurten\{}\tijdelijk\panden.tif").format(
            x)  # provide a default value if unspecified

    panden_0 = arcpy.GetParameterAsText(14)
    if panden_0 == '#' or not panden_0:
        panden_0 = os.path.join(work_location, r"Buurten\{}\tijdelijk\panden_0.tif").format(
            x)  # provide a default value if unspecified
    ...
    if resultaten == '#' or not resultaten:
        resultaten = os.path.join(work_location, r"Buurten\{}\resultaten").format(
            x)  # provide a default value if unspecified

    Sloten = arcpy.GetParameterAsText(12)
    if Sloten == '#' or not Sloten:
        Sloten = os.path.join(work_location, r"Buurten\{}\tijdelijk\Sloten.tif").format(
            x)  # provide a default value if unspecified

    Sloten_Int = arcpy.GetParameterAsText(12)
    if Sloten_Int == '#' or not Sloten_Int:
        Sloten_Int = os.path.join(work_location, r"Buurten\{}\tijdelijk\Sloten_Int.tif").format(
            x)  # provide a default value if unspecified
    ...
    if waterOpStraat == '#' or not waterOpStraat:
        waterOpStraat = os.path.join(work_location, r"Buurten\{}\resultaten\waterOpStraat.tif").format(
            x)  # provide a default value if unspecified
    ...
    if wos_0 == '#' or not wos_0:
        wos_0 = os.path.join(work_location, r"Buurten\{}\resultaten\wos_0.tif").format(
            x)  # provide a default value if unspecified

    water_0 = arcpy.GetParameterAsText(15)
    if water_0 == '#' or not water_0:
        water_0 = os.path.join(work_location, r"Buurten\{}\tijdelijk\water_0.tif").format(
            x)  # provide a default value if unspecified
    ...
    Stream_lines_poly_minWOS_shp = os.path.join(work_location,
                                                r"Buurten\{}\resultaten\Stream_lines_poly_minWOS.shp").format(x)
    ...
    arcpy.PolygonToRaster_conversion(
        os.path.join(work_location, r"Buurten\{}\tijdelijk\pandPolygon_clip.shp").format(x), "FID", panden,
    ...
    arcpy.Shift_management(BK_AHN, os.path.join(work_location, r"Buurten\{}\tijdelijk\BK_shift.tif").format(x), "0",
                           "0", panden_max)
    ...
    for y in range(1, 3):
        ...
        if y == 1:
            ...
            arcpy.MosaicToNewRaster_management([water_full], resultaten, "w_complete.tif", "", "32_BIT_FLOAT", "", "1",
                                               "LAST", "FIRST")
        ...
        arcpy.MosaicToNewRaster_management([BK_shift, water_min], resultaten, "BK_mW.tif", "", "32_BIT_FLOAT", "", "1",
                                           "MINIMUM", "FIRST")

        # -------Interpolating the AHN-----------------
        # Process: Raster Calculator (3) Interpolation of the raster
        out_raster = FocalStatistics(Raster(os.path.join(work_location, r"Buurten\{}\resultaten\BK_mW.tif").format(x)),
                                     NbrCircle(6, "CELL"), "MEAN", "")
        out_raster.save(os.path.join(work_location, r"Buurten\{}\tijdelijk\bk_focal.tif").format(x))
        BK_mW_NdV6 = Con(IsNull(os.path.join(work_location, r"Buurten\{}\resultaten\BK_mW.tif").format(x)), bk_focal,
                         Raster(os.path.join(work_location, r"Buurten\{}\resultaten\BK_mW.tif").format(x)))
        BK_mW_NdV6.save(os.path.join(work_location, r"Buurten\{}\tijdelijk\BK_mW_NdV6.tif").format(x))

        out_raster = FocalStatistics(
            Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\BK_mW_NdV6.tif").format(x)), NbrCircle(6, "CELL"),
            "MEAN", "")
        out_raster.save(os.path.join(work_location, r"Buurten\{}\tijdelijk\bk_focal2.tif").format(x))
        BK_mW_NdV62 = Con(IsNull(os.path.join(work_location, r"Buurten\{}\tijdelijk\BK_mW_NdV6.tif").format(x)),
                          bk_focal2,
                          Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\BK_mW_NdV6.tif").format(x)))
        BK_mW_NdV62.save(os.path.join(work_location, r"Buurten\{}\tijdelijk\BK_mW_NdV62.tif").format(x))

        out_raster = FocalStatistics(
            Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\BK_mW_NdV62.tif").format(x)),
            NbrCircle(6, "CELL"), "MEAN", "")
        out_raster.save(os.path.join(work_location, r"Buurten\{}\tijdelijk\bk_focal3.tif").format(x))
        BK_mW_NdV63 = Con(IsNull(os.path.join(work_location, r"Buurten\{}\tijdelijk\BK_mW_NdV62.tif").format(x)),
                          bk_focal3,
                          Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\BK_mW_NdV62.tif").format(x)))
        BK_mW_NdV63.save(os.path.join(work_location, r"Buurten\{}\tijdelijk\BK_mW_NdV63.tif").format(x))

        out_raster = FocalStatistics(
            Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\BK_mW_NdV63.tif").format(x)),
            NbrCircle(6, "CELL"), "MEAN", "")
        out_raster.save(os.path.join(work_location, r"Buurten\{}\tijdelijk\bk_focal4.tif").format(x))
        BK_AHN_mW = Con(IsNull(os.path.join(work_location, r"Buurten\{}\tijdelijk\BK_mW_NdV63.tif").format(x)),
                        bk_focal4,
                        Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\BK_mW_NdV63.tif").format(x)))
        ...
        arcpy.MosaicToNewRaster_management([BK_AHN_mW, panden_max], resultaten, "BK_AHN_mWmP.tif", "", "32_BIT_FLOAT",
                                           "", "1", "MAXIMUM", "FIRST")
        ...
        if y == 1:
            ...
            arcpy.Shift_management(mpzw, os.path.join(work_location, r"Buurten\{}\tijdelijk\mpzw_shift.tif").format(x),
                                   "0", "0", panden_max)
        else:
            # Process: Is Null
        ...
        Sloten = Con(Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\outisNull.tif").format(x)) == 1, Con(
            Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\mpzw_sinks.tif").format(
                x)) >= WaterdiepteNieuwSloten, mpzw_sinks, ""))
        ...
        outRgnGrp = RegionGroup(os.path.join(work_location, r"Buurten\{}\tijdelijk\Sloten_Int.tif").format(x),
                                "FOUR")  # Create separate groups
    ...
    waterOpStraat = Con(Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\mpzw_sinks.tif").format(x)) >= 0.04,
                        Con(Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\mpzw_sinks.tif").format(x)) <= 3,
                            mpzw_sinks, ""))
    ...
    mPzW_l10_g1 = Con(Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\mPzW_fa_log10.tif").format(x)) >= 1,
                      Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\mPzW_fa_log10.tif").format(x)), "")
    ...
    arcpy.MosaicToNewRaster_management([wos_0, panden_0, mpzw_strorder, water_0], resultaten, "BK_streamOrder.tif", "",
    ...
    arcpy.gp.Reclassify_sa(str_l_int, "VALUE", "0 NODATA;1 NODATA;2 NODATA;3 3;4 4;5 5;6 6;7 7;8 8", str_l_reclass,
                           "DATA")
    ...
    arcpy.Buffer_analysis(pandPolygon_shp,
                          os.path.join(work_location, r"Buurten\{}\tijdelijk\pandPolygon_Buffer").format(x),
                          "0.25 Meter", "FULL", "ROUND", "NONE")

    # Process: Select all buildings which overlap with their buffer and the highest level of Water on the streets
    outZonalStatstable = ZonalStatisticsAsTable(
        os.path.join(work_location, r"Buurten\{}\tijdelijk\pandPolygon_Buffer.shp").format(x), "FID",
        os.path.join(work_location, r"Buurten\{}\resultaten\waterOpStraat.tif").format(x), "Table2", "DATA", "MAXIMUM")
    ...
    arcpy.SelectLayerByAttribute_management("pandPolygon_lyr", "NEW_SELECTION",
                                            "AREA>=0.75")  # Only buildings that overlap with WOS >1m2 (on the outside buffered area of the building)
    ...
    waterVoorStreamlines = Con(
        Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\mpzw_sinks.tif").format(x)) >= WaterdiepteStreamlines,
        mpzw_sinks, "")
    ...
    mpzw_half = Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\mpzw_fill.tif").format(x)) - Raster(
        os.path.join(work_location, r"Buurten\{}\resultaten\waterVoorStreamlines.tif").format(x))
    mpzw_half.save(os.path.join(work_location, r"Buurten\{}\tijdelijk\mpzw_half.tif").format(
        x))  # only contains water on the streets above 0.4
    ...
    arcpy.MosaicToNewRaster_management([mpzw_half, mpzw_fill], resultaten, "mPzW_minWOS.tif", "", "32_BIT_FLOAT", "",
                                       "1", "FIRST",
                                       "FIRST")  # Maakt een volledige hoogtekaart (daarvoor heb je mpzw_fill nodig), gevuld tot aan 0.4 en niet hoger.
    arcpy.Shift_management(mPzW_minWOS,
                           os.path.join(work_location, r"Buurten\{}\tijdelijk\mPzW_minWOSshift.tif").format(x), "0",
                           "0", panden_max)
    # Process: Create Streamlines. Same process as above.
    arcpy.gp.FlowDirection_sa(os.path.join(work_location, r"Buurten\{}\tijdelijk\mPzW_minWOSshift.tif").format(x),
                              mPzW_fd_minWOS, "NORMAL", output_drop2)
    ...
    mPzW_l10_g1_minWOS = Con(
        Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\mPzW_fa_log10_minWOS.tif").format(x)) >= 1,
        Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\mPzW_fa_log10_minWOS.tif").format(x)), "")
    ...
    arcpy.MosaicToNewRaster_management([wos_0, panden_0, mpzw_strorder_minWOS, water_0], resultaten,
                                       "BK_streamOrder_minWOS.tif", "", "32_BIT_FLOAT", "", "1", "MINIMUM", "FIRST")
    arcpy.gp.Int_sa(BK_streamOrder_minWOS, str_l_int_minWOS)
    arcpy.gp.Reclassify_sa(str_l_int_minWOS, "VALUE", "0 NODATA;1 NODATA;2 NODATA;3 3;4 4;5 5;6 6;7 7;8 8",
                           str_l_reclass_minWOS, "DATA")
    arcpy.RasterToPolyline_conversion(str_l_reclass_minWOS, Stream_lines_poly_minWOS_shp, "ZERO", "0", "SIMPLIFY",
                                      "VALUE")
    ...
    Volumes_Mosaic = Raster(os.path.join(work_location, r"Buurten\{}\tijdelijk\mpzw_shift.tif").format(x)) + Raster(
        os.path.join(work_location, r"Buurten\{}\resultaten\waterOpStraat.tif").format(x))
    ...
    Volumes_WOS = ExtractByAttributes(os.path.join(work_location, r"Buurten\{}\tijdelijk\OutCutFill.tif").format(x),
                                      "VOLUME>0.01 AND AREA>0.5")  # Only areas with more than 0.50m2 water
    Volumes_WOS.save(os.path.join(work_location, r"Buurten\{}\tijdelijk\Volumes_WOS.tif").format(x))

    arcpy.RasterToPolygon_conversion(Volumes_WOS,
                                     os.path.join(work_location, r"Buurten\{}\tijdelijk\Volumes1_WOS_shp.shp").format(
                                         x), "NO_SIMPLIFY", "Value")

    ITERATIONS = ['Volumes1', 'Volumes2', 'Volumes3', 'Volumes4', 'Volumes5', 'Volumes6', 'Volumes7', 'Volumes8',
                  'Volumes9', 'Volumes0']
    ...
    for x in ITERATIONS:
        ...
        Volumes1_points_shift = os.path.join(work_location, r"Buurten", _buurt,
                                             r"tijdelijk\{}_points_shift.tif").format(x)
        ...
        arcpy.RasterToPolygon_conversion(Volumes1_Basin, os.path.join(work_location, r"Buurten", _buurt,
                                                                      r"tijdelijk\{}_Basin_shp.shp").format(x),
                                         "NO_SIMPLIFY")
        ...
        arcpy.Intersect_analysis(
            [os.path.join(work_location, r"Buurten", _buurt, r"tijdelijk\{}_WOS_shp.shp").format(x),
             os.path.join(work_location, r"Buurten", _buurt, r"tijdelijk\{}_Basin_Area.shp").format(x)],
            Volumes1_Intersect, "", "", "INPUT")
        ...
        if x == 'Volumes1':
            arcpy.CalculateField_management(Volumes1_WOS_shp, "{}_B".format(x),
                                            "(!SUM_F_AREA!*(" + str(Stapgrootte) + " - ((" + str(Riool) + "/10)*" + str(
                                                uur) + ")))", "PYTHON_9.3")
            ...
            arcpy.CopyFeatures_management(Volumes1_WOS_lyr, os.path.join(work_location, r"Buurten", _buurt,
                                                                         r"tijdelijk\Volumes2_WOS_shp.shp"))
        if x == 'Volumes2':
            arcpy.CalculateField_management(Volumes1_WOS_shp, "{}_B".format(x),
                                            "(!SUM_F_AR_1!*(" + str(Stapgrootte) + " - ((" + str(Riool) + "/10)*" + str(
                                                uur) + ")))", "PYTHON_9.3")
            arcpy.JoinField_management(Volumes1_WOS_shp, "gridcode", Volumes_WOS, "Value", "VOLUME")
            # 5 Select all the Volumes that are not filled (0.9)
            arcpy.MakeFeatureLayer_management(Volumes1_WOS_shp, Volumes1_WOS_lyr)
            arcpy.SelectLayerByAttribute_management(
                os.path.join(work_location, r"Buurten", _buurt, r"tijdelijk\Volumes2_WOS_lyr"), "NEW_SELECTION",
                "VOLUME*0.9>(Volumes1_B+Volumes2_B)")
            arcpy.CopyFeatures_management(Volumes1_WOS_lyr, os.path.join(work_location, r"Buurten", _buurt,
                                                                         r"tijdelijk\Volumes3_WOS_shp.shp"))
        if x == 'Volumes3':
            arcpy.CalculateField_management(Volumes1_WOS_shp, "{}_B".format(x),
                                            "(!SUM_F_AR_2!*(" + str(Stapgrootte) + " - ((" + str(Riool) + "/10)*" + str(
                                                uur) + ")))", "PYTHON_9.3")
            arcpy.JoinField_management(Volumes1_WOS_shp, "gridcode", Volumes_WOS, "Value", "VOLUME")
            # 5 Select all the Volumes that are not filled (0.9)
            arcpy.MakeFeatureLayer_management(Volumes1_WOS_shp, Volumes1_WOS_lyr)
            arcpy.SelectLayerByAttribute_management(
                os.path.join(work_location, r"Buurten", _buurt, r"tijdelijk\Volumes3_WOS_lyr"), "NEW_SELECTION",
                "VOLUME*0.9>(Volumes1_B+Volumes2_B+Volumes3_B)")
            arcpy.CopyFeatures_management(Volumes1_WOS_lyr, os.path.join(work_location, r"Buurten", _buurt,
                                                                         r"tijdelijk\Volumes4_WOS_shp.shp"))
        if x == 'Volumes4':
            arcpy.CalculateField_management(Volumes1_WOS_shp, "{}_B".format(x),
                                            "(!SUM_F_AR_3!*(" + str(Stapgrootte) + " - ((" + str(Riool) + "/10)*" + str(
                                                uur) + ")))", "PYTHON_9.3")
            arcpy.JoinField_management(Volumes1_WOS_shp, "gridcode", Volumes_WOS, "Value", "VOLUME")
            # 5 Select all the Volumes that are not filled (0.9)
            arcpy.MakeFeatureLayer_management(Volumes1_WOS_shp, Volumes1_WOS_lyr)
            arcpy.SelectLayerByAttribute_management(
                os.path.join(work_location, r"Buurten", _buurt, r"tijdelijk\Volumes4_WOS_lyr"), "NEW_SELECTION",
                "VOLUME*0.9>(Volumes1_B+Volumes2_B+Volumes3_B+Volumes4_B)")
            arcpy.CopyFeatures_management(Volumes1_WOS_lyr, os.path.join(work_location, r"Buurten", _buurt,
                                                                         r"tijdelijk\Volumes5_WOS_shp.shp"))
        if x == 'Volumes5':
            arcpy.CalculateField_management(Volumes1_WOS_shp, "{}_B".format(x),
                                            "(!SUM_F_AR_4!*(" + str(Stapgrootte) + " - ((" + str(Riool) + "/10)*" + str(
                                                uur) + ")))", "PYTHON_9.3")
            arcpy.JoinField_management(Volumes1_WOS_shp, "gridcode", Volumes_WOS, "Value", "VOLUME")
            # 5 Select all the Volumes that are not filled (0.9)
            arcpy.MakeFeatureLayer_management(Volumes1_WOS_shp, Volumes1_WOS_lyr)
            arcpy.SelectLayerByAttribute_management(
                os.path.join(work_location, r"Buurten", _buurt, r"tijdelijk\Volumes5_WOS_lyr"), "NEW_SELECTION",
                "VOLUME*0.9>(Volumes1_B+Volumes2_B+Volumes3_B+Volumes4_B+Volumes5_B)")
            arcpy.CopyFeatures_management(Volumes1_WOS_lyr, os.path.join(work_location, r"Buurten", _buurt,
                                                                         r"tijdelijk\Volumes6_WOS_shp.shp"))
        if x == 'Volumes6':
            arcpy.CalculateField_management(Volumes1_WOS_shp, "{}_B".format(x),
                                            "(!SUM_F_AR_5!*(" + str(Stapgrootte) + " - ((" + str(Riool) + "/10)*" + str(
                                                uur) + ")))", "PYTHON_9.3")
            arcpy.JoinField_management(Volumes1_WOS_shp, "gridcode", Volumes_WOS, "Value", "VOLUME")
            # 5 Select all the Volumes that are not filled (0.9)
            arcpy.MakeFeatureLayer_management(Volumes1_WOS_shp, Volumes1_WOS_lyr)
            arcpy.SelectLayerByAttribute_management(
                os.path.join(work_location, r"Buurten", _buurt, r"tijdelijk\Volumes6_WOS_lyr"), "NEW_SELECTION",
                "VOLUME*0.9>(Volumes1_B+Volumes2_B+Volumes3_B+Volumes4_B+Volumes5_B+Volumes6_B)")
            arcpy.CopyFeatures_management(Volumes1_WOS_lyr, os.path.join(work_location, r"Buurten", _buurt,
                                                                         r"tijdelijk\Volumes7_WOS_shp.shp"))
        if x == 'Volumes7':
            arcpy.CalculateField_management(Volumes1_WOS_shp, "{}_B".format(x),
                                            "(!SUM_F_AR_6!*(" + str(Stapgrootte) + " - ((" + str(Riool) + "/10)*" + str(
                                                uur) + ")))", "PYTHON_9.3")
            arcpy.JoinField_management(Volumes1_WOS_shp, "gridcode", Volumes_WOS, "Value", "VOLUME")
            # 5 Select all the Volumes that are not filled (0.9)
            arcpy.MakeFeatureLayer_management(Volumes1_WOS_shp, Volumes1_WOS_lyr)
            arcpy.SelectLayerByAttribute_management(
                os.path.join(work_location, r"Buurten", _buurt, r"tijdelijk\Volumes7_WOS_lyr"), "NEW_SELECTION",
                "VOLUME*0.9>(Volumes1_B+Volumes2_B+Volumes3_B+Volumes4_B+Volumes5_B+Volumes6_B+Volumes7_B)")
            arcpy.CopyFeatures_management(Volumes1_WOS_lyr, os.path.join(work_location, r"Buurten", _buurt,
                                                                         r"tijdelijk\Volumes8_WOS_shp.shp"))
        if x == 'Volumes8':
            arcpy.CalculateField_management(Volumes1_WOS_shp, "{}_B".format(x),
                                            "(!SUM_F_AR_7!*(" + str(Stapgrootte) + " - ((" + str(Riool) + "/10)*" + str(
                                                uur) + ")))", "PYTHON_9.3")
            arcpy.JoinField_management(Volumes1_WOS_shp, "gridcode", Volumes_WOS, "Value", "VOLUME")
            # 5 Select all the Volumes that are not filled (0.9)
            arcpy.MakeFeatureLayer_management(Volumes1_WOS_shp, Volumes1_WOS_lyr)
            arcpy.SelectLayerByAttribute_management(
                os.path.join(work_location, r"Buurten", _buurt, r"tijdelijk\Volumes8_WOS_lyr"), "NEW_SELECTION",
                "VOLUME*0.9>(Volumes1_B+Volumes2_B+Volumes3_B+Volumes4_B+Volumes5_B+Volumes6_B+Volumes7_B+Volumes8_B)")
            arcpy.CopyFeatures_management(Volumes1_WOS_lyr, os.path.join(work_location, r"Buurten", _buurt,
                                                                         r"tijdelijk\Volumes9_WOS_shp.shp"))
        if x == 'Volumes9':
            arcpy.CalculateField_management(Volumes1_WOS_shp, "{}_B".format(x),
                                            "(!SUM_F_AR_8!*(" + str(Stapgrootte) + " - ((" + str(Riool) + "/10)*" + str(
                                                uur) + ")))", "PYTHON_9.3")
            arcpy.JoinField_management(Volumes1_WOS_shp, "gridcode", Volumes_WOS, "Value", "VOLUME")
            # 5 Select all the Volumes that are not filled (0.9)
            arcpy.MakeFeatureLayer_management(Volumes1_WOS_shp, Volumes1_WOS_lyr)
            arcpy.SelectLayerByAttribute_management(
                os.path.join(work_location, r"Buurten", _buurt, r"tijdelijk\Volumes9_WOS_lyr"), "NEW_SELECTION",
                "VOLUME*0.9>(Volumes1_B+Volumes2_B+Volumes3_B+Volumes4_B+Volumes5_B+Volumes6_B+Volumes7_B+Volumes8_B+Volumes9_B)")
            arcpy.CopyFeatures_management(Volumes1_WOS_lyr, os.path.join(work_location, r"Buurten", _buurt,
                                                                         r"tijdelijk\Volumes0_WOS_shp.shp"))
        if x == 'Volumes0':
            arcpy.CalculateField_management(Volumes1_WOS_shp, "{}_B".format(x),
                                            "(!SUM_F_AR_9!*(" + str(Stapgrootte) + " - ((" + str(Riool) + "/10)*" + str(
                                                uur) + ")))", "PYTHON_9.3")
            arcpy.JoinField_management(Volumes1_WOS_shp, "gridcode", Volumes_WOS, "Value", "VOLUME")
            # 5 Select all the Volumes that are not filled (0.9)
            arcpy.MakeFeatureLayer_management(Volumes1_WOS_shp, Volumes1_WOS_lyr)
            arcpy.SelectLayerByAttribute_management(
                os.path.join(work_location, r"Buurten", _buurt, r"tijdelijk\Volumes0_WOS_lyr"), "NEW_SELECTION",
                "VOLUME*0.9>(Volumes1_B+Volumes2_B+Volumes3_B+Volumes4_B+Volumes5_B+Volumes6_B+Volumes7_B+Volumes8_B+Volumes9_B+Volumes0_B)")
            arcpy.CopyFeatures_management(Volumes1_WOS_lyr, os.path.join(work_location, r"Buurten", _buurt,
                                                                         r"resultaten\Volumes0_Final.shp"))
            arcpy.AddField_management(os.path.join(work_location, r"Buurten", _buurt, r"resultaten\Volumes0_Final.shp"),
                                      "Fill_Per", "DOUBLE")
            arcpy.CalculateField_management(
                os.path.join(work_location, r"Buurten", _buurt, "resultaten\Volumes0_Final.shp"), "Fill_Per",
                "(!Volumes1_B!+!Volumes2_B!+!Volumes3_B!+!Volumes4_B!+!Volumes5_B!+!Volumes6_B!+!Volumes7_B!+!Volumes8_B!+!Volumes9_B!+!Volumes0_B!)/!Volume!",
                "PYTHON_9.3")
